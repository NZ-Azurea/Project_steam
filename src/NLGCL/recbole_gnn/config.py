...
class Config(RecBole_Config):
    ...
    def compatibility_settings(self):
        import numpy as np
        np.bool = np.bool_           # np.bool_ existe encore
        np.int = np.int64            # np.int_ remplacé par np.int64
        np.float = np.float64        # np.float_ supprimé → np.float64
        np.complex = np.complex128   # np.complex_ supprimé → np.complex128
        np.object = object           # np.object_ supprimé → Python object
        np.str = str                 # np.str_ supprimé → Python str
        np.long = np.int64           # np.long n’existe plus → np.int64
        np.unicode = str             # np.unicode supprimé → Python str
